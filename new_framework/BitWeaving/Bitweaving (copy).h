...
size_t BitWeavingBase::write_file_to_bitweaving(Scan &scan, uint32_t dimension, uint32_t num_samples)
{
	...
	while (scan.HasNext()) 
	{
        ...
        // finish off the previous vector and start a new one
			...
			uint32_t *d = new uint32_t[data.size()];
			...
			hazy::vector::SVector<uint32_t> sparse_vector(d, i, data.size());
			...
			hazy::vector::ScaleAndAdd( temp_vector, sparse_vector, 1 );
			...
			for (int ii = 0; ii < dimension; ii++)
			{
				...
				if (ii == i[nonzero_index])
				{
					if ( (d[nonzero_index]>>24) != dest[ii] )
					{
						printf("The data is not right_d[%d] = 0x%x, dest[%d] = 0x%x\n", nonzero_index, d[nonzero_index], ii, dest[ii]);
						return 2;
					}
					nonzero_index++;
				}
				else
				{
					if ( 0 != dest[ii] )
					{
						printf("The data is not right_dest[%d] = 0x%x\n", ii, dest[ii]);
						return 2;
					}
