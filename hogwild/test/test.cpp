...
/*
            v_sum =                        _mm256_blend_epi32 (v_00, v_8_ff, sum_array[7]) ; //8-bit
        v_sum = _mm256_or_si256(v_sum, _mm256_blend_epi32 (v_00, v_7_ff, sum_array[6])); //7-bit
        v_sum = _mm256_or_si256(v_sum, _mm256_blend_epi32 (v_00, v_6_ff, sum_array[5])); //6-bit
        v_sum = _mm256_or_si256(v_sum, _mm256_blend_epi32 (v_00, v_5_ff, sum_array[4])); //5-bit
        v_sum = _mm256_or_si256(v_sum, _mm256_blend_epi32 (v_00, v_4_ff, sum_array[3])); //4-bit
        v_sum = _mm256_or_si256(v_sum, _mm256_blend_epi32 (v_00, v_3_ff, sum_array[2])); //3-bit
        v_sum = _mm256_or_si256(v_sum, _mm256_blend_epi32 (v_00, v_2_ff, sum_array[1])); //2-bit
        v_sum = _mm256_or_si256(v_sum, _mm256_blend_epi32 (v_00, v_1_ff, sum_array[0])); //1-bit


    //int sum_array[8];
    _mm256_store_si256((__m256i *)sum_array, v_sum);

    printf("Back to the orignal format...\n");
    for (int i = 0; i < 8; i += 1) {
