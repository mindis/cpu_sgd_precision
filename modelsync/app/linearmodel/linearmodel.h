...
#define NUM_BYTES_FOR_EACH_COUNTER 128
...
struct LinearModel
{
  ...
  size_t nthreads_;
  size_t num_threads_2_pow_;
  unsigned *flag_counter;


  inline unsigned& get_flag_counter(unsigned idx) {
    return flag_counter[idx*NUM_BYTES_FOR_EACH_COUNTER];
  }
  ...
  explicit LinearModel(unsigned dim, unsigned nthreads)
  {
	...
	flag_counter = new unsigned [nthreads*128]; //two cache line for each thread.
	for(unsigned i = 0; i < nthreads; i++)
		get_flag_counter(i) = 0;
	
	size_t num_threads = nthreads, num_threads_2_pow;
	...
	num_threads_2_pow_ = num_threads_2_pow;
